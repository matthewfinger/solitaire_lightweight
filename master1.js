...
const colWidth = Math.trunc(width / 9); //const for the empty card img to be shown at the base of piles

const emptCardImg = new Image();
emptCardImg.src = 'images/empty-card.png'; //const for the img to be shown when the deck is all the way drawn.

const emptDeckImg = new Image();
emptDeckImg.src = 'images/empty-deck.png';
var mousex = 0;
var mousey = 0;
var globalGame; //pwidth and pHeight return a percentage of the width and height based on a percentage p
...
const Suits = [new Suit('hearts', 'red', 'H'), new Suit('diamonds', 'red', 'D'), new Suit('clubs', 'black', 'C'), new Suit('spades', 'black', 'S')];
...
class InterfaceSettings {
  constructor(root) {
    ...
    this.mainSection.id = "InterfaceSettings";
...
class Game {
  constructor(root) {
    ...
    this.score = 0;
    this.done = false;
    globalGame = this; //setting up new game!
    //first, shuffle the Deck

    this.mainDeck.shuffle(); //7 main piles
    ...
    document.onmousedown = event => {
      if (!globalGame.InterfaceSettings.getValue('drag')) {
        ...
        h = p.isOver;
        let g = globalGame;
        let s; //is the mouse even over anything?
        ...
        if (typeof h.value == 'number') {
          ...
          if (p.hasCards()) {
            ...
            if (s[0] > 0 && s[0] < 8) {
              //if the player is over one of the 7 main piles
              if (vd == -1 && c.suit.color != h.suit.color) {
                let crds = p.dropCards();
                s[2].push(crds);
              }
            } else {
              //when the player is over one of the suit stacks
              if (p.cardStack.cards.length == 1) {
                // c is the only card in p
                if (vd === 1 && c.suit.name === h.suit.name) {
                  //if the card is one greater and of the same suit
            ...
            if (s[0] == 0) {
              // when the player is over the first column
              if (s[1] == 0) {
                //when player is on mainDeck, draw three cards
                g.draw();
              } else if (s[2].cards[s[2].cards.length - 1] == p.isOver) {
                //the player is over the TOP card of the hand
                g.player.grabCards(s[2].pop(1), s[2]);
              }
            } else if (s[0] < 8) {
              //when the player is over one of the 7 main stacks
              if (h.faceup) {
                //the card hovered over is face up
                let popcount = h.value - s[2].cards[s[2].cards.length - 1].value + 1;
                g.player.grabCards(s[2].pop(popcount), s[2]);
              }
            } else {
              //when player is over one of the suit stacks
              if (s[2].cards.length > 1) {
                //making sure there's a card to grab!
                g.player.grabCards(s[2].pop(1), s[2]);
              }
            }
          }
        } else if (g.isOverStackBase(p)) {
          //if the player is over an empty base, rather than an actual card
          s = g.isOverStackBase(p);

          if (s.cards.length != 0) {
            return;
          } else if (s == g.mainDeck && !p.hasCards()) {
            //if the user clicks on the empty pile && does not have a card, move hand to deck with draw method
            g.draw();
            g.evaluate();
            return;
  ...
  draw() {
    ...
    if (l != 0) {
      //draw up to three cards from maindeck to hand
      ...
      if (this.handStack.cards.length != 0) {
        //if no cards in deck, but cards in hand, move hand to deck
        sounds['low-tick'].play();
        this.handStack.flip();
        this.mainDeck.push(this.handStack.pop(this.handStack.cards.length)); //reverse the order of the cards
  ...
  drawStack(stack, col) {
    let x;
    let y; //if col is not a valid column

    if (col === false) {
      //remember, in the following block of code, when used correctly, stack actually refers to a player object
      x = stack.x;
      y = stack.y;
      stack = stack.cardStack;
    } else if (col === true) {
      //when drawing the hand stack
      x = 5;
      y = stack.y;
      ...
      //otherwise, when fun used correctly, stack:Stack and col:num in game.colstacks
      x = (col - 1) * colWidth + 5;
      y = stack.y;
  ...
  evaluate(flag = false) {
    ...
    for (let i = 0; i < this.cardStacks.length; i++) {
      ...
      if (!this.cardStacks[i].cards[this.cardStacks[i].cards.length - 1].faceup) {
        //if the top card in the stack is face down
  ...
  playerAt(player) {
    let x = player.x;
    let y = player.y;
    let stacks = this.stackCols[Math.trunc(x / colWidth)]; //which (if any) of the columns is the player in?
    ...
    if (x % colWidth > 5 && x % colWidth < colWidth - 5) {
      ...
      for (let i = 0; i < stacks.length; i++) {
        if (stacks[i].y <= y && stacks[i].y + stacks[i].height() >= y) {
          ...
          if (stacks[i] == this.handStack) {
            //only the TOP three cards are visible in handstack
            ind = Math.min(ind, 2);
  ...
  refresh() {
    ...
    this.renderGame(); //if stop is true, do not refresh again
  ...
  renderGame() {
    ...
    this.ctx.clearRect(0, 0, width, height); //then, render all the stacks in colstacks

    for (let i = 0; i < this.stackCols.length; i++) {
      for (let j = 0; j < this.stackCols[i].length; j++) {
        if (i === 0 && j === 1) {
          //for the hand stack, we only want the top three cards!
